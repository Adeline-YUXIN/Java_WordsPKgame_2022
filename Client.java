...
public class Client extends JFrame implements Runnable,ActionListener {
    Color c = new Color(78, 150, 149);                         //设置背景颜色
    ...
    public Client() throws Exception { //图形界面
        this.setSize(650, 600);                       //设置界面大小
        this.setLocation(640,80);                           //设置界面位置
        ...
        while (true) {
            try {
                ...
                if(msg.startsWith("生命值:"))
                {
                    ...
                    fos1.write(Wordtofile.getBytes());//按当前文件的编码方式编码，防止出现乱码
                    fos1.write("（未答）\n".getBytes());
                    fos1.close();
            ...
            while (true) {
                try {
                    ...
                    if (Y < 510) {
                        ...
                        ps.println("over");
                        ps.println("start");
                        Y = 1000;//相当于针对这个单词的线程结束了（不会再进入while循环）
